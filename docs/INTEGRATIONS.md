...
ReputationBan 0.21.0 では LuckPerms、CoreProtect、WorldGuard、GriefPrevention、PlaceholderAPI、DiscordSRV を任意連携として扱います。いずれも `softdepend` であり、未導入、未ロード、API unavailable の場合でも ReputationBan 単体の通報、監査、BAN、backup、support bundle は継続します。
...
placeholder 呼び出し中に SQLite へ同期問い合わせは行いません。値は online player summary cache から返します。cache は join 後、定期 refresh、主要な score 変更後に更新しますが、Phase 21 では最大 `cache-refresh-seconds` 程度の遅延があり得ます。cache 未取得、offline、player なしの場合は `show-unknown-as` を返します。ただし `version` は player なしでも返します。
...
Java ソースでは DiscordSRV API と JDA API を直接 import せず、`DiscordSrvReflectionAdapter` が `github.scarsz.discordsrv.DiscordSRV`、`getPlugin()`、`getAccountLinkManager()`、`getDiscordId(UUID)` を reflection で候補 lookup します。通知では `getDestinationTextChannelForGameChannelName(channel)`、`sendMessage(String)`、`queue()` を reflection で呼びます。API 形状が違う場合は unavailable として扱い、本体処理は失敗させません。
...
DiscordSRV 通知は既存の Discord Webhook 通知とは別経路です。Webhook は URL に送信しますが、DiscordSRV 通知は DiscordSRV の bot と game channel 解決を利用します。二重通知を避けるため、`integrations.discordsrv.notifications.enabled` はデフォルト `false` です。Phase 21 では `report-created` と `auto-ban` を含む event toggle を実装していますが、理由は `include-reasons=false` で削除し、メッセージは短く truncate します。
