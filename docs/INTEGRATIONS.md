...
ReputationBan 1.0.1 では LuckPerms、CoreProtect、WorldGuard、GriefPrevention、PlaceholderAPI、DiscordSRV を任意連携として扱います。いずれも `softdepend` であり、未導入、未ロード、API unavailable の場合でも ReputationBan 単体の通報、監査、BAN、backup、support bundle は継続します。
...
`bypass-groups` は `reputationban.bypass` 権限と OP 保護に加える補助保護です。対象プレイヤーの primary group が `admin` や `owner` などに一致した場合、通報、減点、自動 BAN の対象外として扱います。Phase 37 以降は TargetProtectionService が `/reportbad`、`/reports approve`、BANしきい値を跨ぐ `/rep remove|set`、`PunishmentService#punishIfNeeded` で同じ保護判定を使います。

`offline-lookup.enabled: true` の場合、保護判定に限り LuckPerms `loadUser(UUID)` を reflection 経由で呼び、未ロードの offline user も bypass group として守ります。main thread は future 完了待ちでブロックしません。`timeout-millis` を超えた場合、`fail-closed-for-bypass: true` なら保護対象として扱い、false なら従来通り unavailable/default 扱いです。
...
placeholder 呼び出し中に SQLite へ同期問い合わせは行いません。値は online player summary cache から返します。cache は join 後、定期 refresh、主要な score 変更後に更新しますが、最大 `cache-refresh-seconds` 程度の遅延があり得ます。cache 未取得、offline、player なしの場合は `show-unknown-as` を返します。ただし `version` は player なしでも返します。

## DiscordSRV

DiscordSRV 連携は、通報者と対象プレイヤーが DiscordSRV の account link を持っているかを審査補助として保存します。DiscordSRV が未導入、未ロード、API unavailable の場合でも ReputationBan 本体は起動し、通報、監査、BAN 処理は継続します。

Java ソースでは DiscordSRV API と JDA API を直接 import せず、`DiscordSrvReflectionAdapter` が Bukkit PluginManager の `getPlugin("DiscordSRV")` で得た plugin instance route を先に試します。plugin instance から `getAccountLinkManager()`、`getDiscordId(UUID)`、`getDestinationTextChannelForGameChannelName(channel)`、`sendMessage(String)`、`queue()` を reflection で呼び、取得できない場合だけ `github.scarsz.discordsrv.DiscordSRV` の `Class.forName` + static `getPlugin()` route を fallback として使います。API 形状が違う場合は unavailable として扱い、本体処理は失敗させません。

保存される provider は `discordsrv` です。summary には `reporterLinked`、`targetLinked`、`discordIds=hidden|visible` が入ります。`include-discord-ids` はデフォルト `false` で、false の場合は Discord ID を保存・表示しません。Discord ID はシークレットではありませんが個人情報に近いため、必要なサーバーだけ明示的に有効化してください。

DiscordSRV 通知は既存の Discord Webhook 通知とは別経路です。Webhook は URL に送信しますが、DiscordSRV 通知は DiscordSRV の bot と game channel 解決を利用します。二重通知を避けるため、`integrations.discordsrv.notifications.enabled` はデフォルト `false` です。Phase 22 では DiscordSRV 通知の状態確認、message sanitize、送信を main thread task 内で行います。`report-created` と `auto-ban` を含む event toggle に対応し、理由は `include-reasons=false` で削除し、メッセージは短く truncate します。

DiscordSRV context は自動 BAN の唯一根拠にしないでください。ReputationBan は DiscordSRV 経由で Discord から Minecraft コマンドを実行しません。Discord ボタン承認、role 付与・剥奪、DiscordSRV の設定変更やチャンネル変更も行いません。

## Commands

- `/rep placeholders`: PlaceholderAPI の連携状態、identifier、利用可能な placeholder 一覧を表示します。
- `/rep integrations`: LuckPerms / CoreProtect / WorldGuard / GriefPrevention / PlaceholderAPI / DiscordSRV の設定値、plugin presence、API availability、active、lookup 設定を表示します。
- `/rep integrations test`: LuckPerms / CoreProtect / WorldGuard / GriefPrevention / PlaceholderAPI / DiscordSRV だけに絞った詳細診断です。破壊的操作、CoreProtect 実 lookup、WorldGuard region/flag 変更、GriefPrevention claim/trust 変更、DiscordSRV test message 送信は行いません。
- `/rep doctor`: ReputationBan 全体診断です。database、tables、config、Discord、backup、連携の簡易状態をまとめて確認します。
- `/reports view <id>`: LuckPerms reporter weight、CoreProtect 証拠サマリー、WorldGuard region context、GriefPrevention claim context、DiscordSRV account link context がある場合に概要を表示します。
...
integrations:
  ...
  discordsrv:
    enabled: true
    account-link-context:
      enabled: true
      include-discord-ids: false
      categories:
        - griefing
        - harassment
        - scam
...
- DiscordSRV API: compile dependency は追加せず、reflection only で扱います。
