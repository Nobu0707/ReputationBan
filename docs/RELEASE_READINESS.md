...
0.16.0 の LuckPerms / CoreProtect 任意連携 release として、次の項目を確認してください。
...
- `./scripts/make-review-archive.sh "Phase 16"` が archive を作成し、`checks/docs-localization.txt` と `checks/latest-paper-runtime-smoke-summary.txt` を含みます。
...
- 可能な限り実Paperサーバーで /rep version、/rep doctor、/rep integrations、/rep support bundle、/rep backup、/reportbad TAB補完を確認してください。
- `config.yml` が生成され、内容を確認済みです。
- `/rep version` が 0.16.0 を表示します。
- `/rep doctor` が database、tables、config、audit export、Discord、backup status を期待通りに表示します。
- `/rep integrations` が LuckPerms / CoreProtect の active、pluginPresent、apiAvailable を表示します。
...
- `build/release/ReputationBan-0.16.0.jar`、`.jar.sha256`、`ReputationBan-0.16.0-release.zip`、`ReputationBan-0.16.0-release.zip.sha256` が存在します。
