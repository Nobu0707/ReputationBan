...
この形式では `reporter=<manual-confirmed>`、`target=<manual-confirmed>`、`reportId=<manual-confirmed>`、`manualConfirmed=true` を summary に残し、同じディレクトリに `manual-checklist.txt` を作成します。
