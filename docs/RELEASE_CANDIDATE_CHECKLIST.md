# v0.26.0 リリース候補チェックリスト

v0.26.0 の player report/evidence runtime smoke gate release candidate として次を確認します。
...
- JAR 名が `ReputationBan-0.26.0.jar` です。
...
- 実施後は `./scripts/record-player-report-runtime-smoke-result.sh --result PASS --reporter <reporter> --target <target> --report-id <id> --note "reportbad and reports evidence passed"` で記録します。
- 実プレイヤー2名以上で確認していない場合は PASS 扱いにしません。未実施なら `./scripts/record-player-report-runtime-smoke-result.sh --result NOT_RUN --note "no two players available"` または summary なしのまま HOLD として扱います。
...
- Integration runtime smoke の PASS または `NOT_RUN` / `FAIL` 理由が明確です。
- Player report/evidence runtime smoke の PASS または `NOT_RUN` / `FAIL` 理由が明確です。
