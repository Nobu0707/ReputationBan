...
- `./scripts/check-discordsrv-runtime-readiness.sh` が成功し、token-configured smoke 未実施なら `HOLD_FOR_DISCORDSRV_CONFIGURED_SMOKE` を表示します。
...
- v1.0.0 公開後は DiscordSRV configured smoke 未実施時に `discordSrvConfiguredSmoke=NOT_RUN` と `judgment=RELEASED_WITH_DISCORDSRV_WARNING` を表示します。
...
- release ZIP に `docs/DISCORDSRV_CONFIGURED_RUNTIME_SMOKE_CHECKLIST.md` が含まれます。
