...
- Phase 16a で reflection adapter を導入し、optional dependency class loading を安全化します。
...
Phase 16a では Java ソース内の直接 import を避け、`LuckPermsReflectionAdapter` と `CoreProtectReflectionAdapter` に reflection 呼び出しを閉じ込めます。これにより LuckPerms 未導入、CoreProtect 未導入、片方だけ導入のいずれでも、外部 API 欠落による `NoClassDefFoundError` で ReputationBan 本体が起動不能にならない設計にしています。

LuckPerms の `getUser(UUID)` が未ロードユーザーで `null` を返す場合、Phase 16 では `default-weight` 扱いになります。将来フェーズで必要に応じて `loadUser` 対応を検討します。

CoreProtect lookup は審査補助のサマリー保存だけに使います。rollback、restore、purge は実行しません。
