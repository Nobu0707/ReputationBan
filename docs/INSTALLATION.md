...
ReputationBan 0.27.0 は PaperMC 26.1.2 と Java 25 を対象にしています。

## 導入条件

- PaperMC 26.1.2 の test server または production server
- Java 25 runtime
- `build/libs/ReputationBan-0.27.0.jar`
- SQLite JDBC は `plugin.yml` の `libraries` から読み込まれます。

## 初回導入

1. Paper server を停止します。
2. `ReputationBan-0.27.0.jar` を server の `plugins/` ディレクトリへ配置します。
