...
この文書は ReputationBan v1.0.0 の最終確認方針です。Phase 29 では final artifact を準備しますが、`v1.0.0` tag 作成と GitHub Release 公開はまだ行いません。次Phaseでユーザーが明示的に承認した後に実行します。
...
4. `scripts/make-review-archive.sh "Phase 29"` で review archive を作成します。
...
Phase 29 では `v1.0.0` tag を作成しません。レビュー承認後の次Phaseで、build/test/release artifact verification と Go/No-Go report を確認してから tag を作成します。
...
Phase 29 では GitHub Release を作成しません。次Phaseでユーザー承認後に release notes final candidate を確認し、必要な文言調整を行ってから GitHub Release を公開します。
...
- ユーザー承認後に `v1.0.0` tag を作成します。
- GitHub Release draft を作成し、release notes を確定します。
- Phase 29 の Go/No-Go report と release notes final candidate をもとに公開内容を最終確認します。
