...
Checks the latest:
  build/manual-smoke/player-report-runtime-*/summary.txt
...
latest_summary() {
  find build/manual-smoke -maxdepth 2 -path '*/player-report-runtime-*/summary.txt' -type f 2>/dev/null \
    | sort \
    | tail -n 1
}
...
case "$RESULT:$STATUS" in
  PASS:*|*:PASS)
    ...
    ;;
  FAIL:*|*:FAIL)
    echo "player report runtime smoke: FAIL"
    echo "summary: $SUMMARY"
    echo "judgment: FAIL_PLAYER_REPORT_RUNTIME_SMOKE"
    [[ "$STRICT" == "1" ]] && exit 1
    exit 0
    ;;
  NOT_RUN:*|*:NOT_RUN)
    echo "player report runtime smoke: NOT_RUN"
    echo "summary: ${SUMMARY:--}"
    echo "judgment: HOLD_FOR_PLAYER_REPORT_RUNTIME_SMOKE"
    [[ "$STRICT" == "1" ]] && exit 1
    exit 0
    ;;
  *)
    echo "player report runtime smoke: $RESULT"
    echo "summary: ${SUMMARY:--}"
    echo "judgment: HOLD_FOR_PLAYER_REPORT_RUNTIME_SMOKE"
    [[ "$STRICT" == "1" ]] && exit 1
    exit 0
