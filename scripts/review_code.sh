...
grep -q 'version = "0.7.0"' build.gradle.kts || fail "build.gradle.kts version is not 0.7.0"
...
grep -R "markActiveBansUnbanned(targetUuid, actor, reason)" src/main/java/dev/modplugin/reputationban/command/RepCommand.java >/dev/null || fail "/rep unban does not pass actor and reason separately"
grep -R "pardon(target.uuid(), target.name(), reason, actor)" src/main/java/dev/modplugin/reputationban/command/RepCommand.java >/dev/null || fail "/rep pardon does not pass reason and actor separately"
